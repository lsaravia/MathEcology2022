...
# ╔═╡ 552930bf-833c-40f0-940e-c4b39e19bbbd
using Plots

# ╔═╡ a3718d23-d28b-4179-a5c1-0711a398732c
using DifferentialEquations

# ╔═╡ 795f9e90-9fe2-11ec-3688-9f8882ed9b87
md"## Modelo Fuegos en el bosque
...
# ╔═╡ b7e55568-3ad8-4f54-b492-d0a3f8703222
md"
...
* El fuego puede extinguirse

  $F \xrightarrow{d_F} Ø$
"

# ╔═╡ 894e67e1-6e56-4775-aa50-37a4589f0b90
md" 
...
* Y podemos definir las siguientes ecuaciones 
   ...
   $\begin{align}
   \frac{Ø}{dt} & = - \lambda_V V Ø + d_F F + d_V V \\
   \frac{F}{dt} & = -d_F F + (\lambda_F F + b_F) V \\
   \frac{V}{dt} & = -(d_V + b_F + \lambda_F F) V + \lambda_V V Ø
   \end{align}$
...
# ╔═╡ d2a866f1-6183-4d50-918e-d244b636c693
md"
...
# ╔═╡ 512a5149-273a-46c1-868f-b0c485736d35
md"
Estas ecuaciones tienen un punto de equilibrio en $(V^\text{abs}, F^\text{abs}) = (0,0)$  
...
cuyos autovalores son $\mu_1^\text{abs} = -d_F$ y $\mu_2^\text{abs} = \lambda_V - b_F - d_V$. 

Por lo tanto, el estado vacío solamente es estable debajo de $\lambda^\text{abs}_V = b_F + d_V$, que es el punto crítico absorvente del sistema. 

Observamos que  $\lambda_F$ no tienen ningún rol en la estabilidad del estado vacío, lo cual es probablemente invalido en un modelo espacialmente explicito..
"

# ╔═╡ 8450c8c4-e779-4bc3-a0b2-fabac49d67ea
function sim_fb_det(N₀,fin_t,h,par)
	λv, dv , λf , bf , df , N = par
	V, F = N₀
	Vs = Float64[V]
	Fs = Float64[F]
	ts = [0.0]
	t = 0.0
	while t ≤ fin_t	
		V_next = V +  h * (λv * V * (N - V - F) - V * (λf * F + dv + bf) )  
		F_next = F +  h * (V * (λf * F + bf) - df * F)
		t_next = t + h 
		push!(Vs,V_next)           # Agregando un elemento al final del vector Ns
		push!(Fs,F_next)           # Agregando un elemento al final del vector Ns
		push!(ts,t_next)
		t = t_next
		V = V_next
		F = F_next
...
# ╔═╡ 06e7efde-6944-4523-9a40-a2f98b21a7e9
begin
X₀ = [100,0]
par = [.0002 , .1, .1, .0001, 1, 1000]
end

# ╔═╡ 9e4162af-f4dd-44ae-87bf-847189c53509
X = sim_fb_det(X₀,500,0.1,par)

# ╔═╡ 7e0fefda-83e6-447b-ad9e-c6c046d8fd45
begin
plot(X[1],X[2],label="V")
plot!(X[1],X[3],label="F")
end

# ╔═╡ 5c0d4809-05d2-40e8-af84-1939b032040d
X1 = [sim_fb_det(X₀,500,0.1,[.0002 , .1, .1, .0001, p, 1000]) for p in range(1,10,5)]

# ╔═╡ 27cc7be2-e5b0-4421-93b4-ab65286dcb92
begin
	plotly()
	plot(X1, label= collect(range(1,10,5))',xlabel="Time",ylabel="V", zlabel="F")
end

# ╔═╡ ecd036a2-35d9-4952-bb56-3f2dd0b76a59
md"## Otra versión del modelo determinístico

* Usamos el paquete de Julia para resolver ecuaciones diferenciales

* Tiene que dar razonablemente lo mismo que antes"

# ╔═╡ 22524301-ce25-4691-a581-b4807e92f108
md"
## Definir las ecuaciones diferenciales"

# ╔═╡ fc021567-0de5-4bb2-843e-265293411b27
function fb_ode!(du, u, p, t)  # ! since modifies du -- in-place updating (can have better performance)
    
	# desempaquetamos parametros y variables: 
    λv , δv, λf, bf , δf, N = p                     
    V, F = u                                        
    
    # define differential equations:
    dF =  (λf * F + bf) * V - δf * F 
    dV =  λv * V * (N - V - F) - (δv + bf + λf * F) * V 
    
    du .= (dV, dF)   # copy the values into the vector du; note the `.`
end


# ╔═╡ f7310198-e14e-433e-9fee-f8e96184bb02
...
# ╔═╡ 01a6b1f5-6200-4180-b8df-062f6f9f2075
X₀

# ╔═╡ f914930a-d3b3-4116-b5fd-6ec03229b38d
#gr(); plot(Xs, label=["V" "F"])
# 
# X₀ son las condiciones iniciales (V,F)
# time_span el intervalo de integracion (simulación)
# params   los mismos parametros que usamos antes
#
# params = λv , δv, λf, bf , δf, N
begin
time_span=[0,500]; params=[.0002 , .1, .1, .0001, 1, 1000]
Xs = sim_fb_ode(X₀,time_span,params)
plot(Xs, label=["V" "F"],vars=(0,1,2))
end


# ╔═╡ c5aeed54-8cea-4c82-b7e9-bee85e348de3
gr(); plot(Xs, label=["V" "F"])

# ╔═╡ 39aa20de-cca6-432f-ad0b-6e107350169e
md"
## Definimos el modelo estocástico de bosque - fuego"

# ╔═╡ dfd22640-141d-4c96-aa27-84e7cb2c3ca4
#
#  Simulación Fuego en el Bosque estocástico
#
function sim_fb_sto(N₀,fin_t,par)
    λv , dv, λf, bf , df, N = par              # desempaquetamos parametros 
    V, F = N₀                                  # desempaquetamos condiciones iniciales
    Ø = N - F - V
	Fs = Float64[F]
    Vs = Float64[V]
    Øs = Float64[Ø]
...
# ╔═╡ 25c1de80-7ba2-4f8b-bcf5-712062b37b70
begin
λv=.0002; dv=.1; λf=.1; bf=0.0001; df=50
Xt = sim_fb_sto([500,0],500,[λv , dv, λf, bf , df, 1000.0])
plot(Xt[1],Xt[2],label="V")
plot!(Xt[1],Xt[3],label="F")
end

# ╔═╡ 89ab4f31-87cd-4381-81d7-40a9dd414d34
X2 = [sim_fb_sto(X₀,500,[λv , dv, λf, bf , p, 1000.0]) for p in range(5,50,5)]

# ╔═╡ d77e1c66-9f29-4f56-8cd6-d7cc283e8ab5
begin
	plotly()
	plot(X2, label= collect(range(5,50,5))',xlabel="Time",ylabel="V", zlabel="F")
end
